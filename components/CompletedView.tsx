...
export const CompletedView: React.FC<CompletedViewProps> = ({
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((t) => {
      ...
      const statusMatch =
        filterStatus === "All" || latest?.status === filterStatus;
  ...
  const getTimeStatus = (task: Task) => {
    ...
    if (latest?.status === "Completed")
      return { label: "Delivered", color: "text-emerald-500" };
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium italic">
              Comprehensive log of all project workstreams
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative z-[70]" ref={statusDropdownRef}>
            <button
              ...
              className="flex items-center gap-3 px-4 py-2.5 bg-white border border-slate-200 rounded-xl hover:border-blue-400 hover:shadow-lg transition-all min-w-[180px] group"
            >
              ...
              <div className="absolute right-0 mt-2 w-full min-w-[220px] bg-white border border-slate-100 rounded-2xl shadow-2xl z-[70] py-2 overflow-hidden border-t-4 border-t-slate-800">
                {statusOptions.map((status) => (
                  <button
                    ...
                    className={`w-full text-left px-5 py-3 text-sm font-bold flex items-center justify-between hover:bg-slate-50 transition-colors ${
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <i
                        className={`fa-solid ${status.icon} ${status.color} text-xs`}
                      ></i>
                      {status.label}
                    </span>
                  </button>
              ...
              <div className="absolute right-0 mt-2 w-full min-w-[220px] bg-white border border-slate-100 rounded-2xl shadow-2xl z-[65] py-2 overflow-hidden border-t-4 border-t-slate-800">
                {categoryOptions.map((cat) => (
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <i
                        className={`fa-solid ${cat.icon} text-xs opacity-50`}
                      ></i>
                      {cat.label}
                    </span>
                  </button>
          ...
          <div className="relative z-[60]" ref={userDropdownRef}>
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-sm font-black text-slate-800 truncate">
                  {filterUser === "All" ? "Full Team" : filterUser}
                </p>
              </div>
            </button>
            {isUserDropdownOpen && (
              <div className="absolute right-0 mt-2 w-full min-w-[220px] bg-white border border-slate-100 rounded-2xl shadow-2xl z-[60] py-2 overflow-hidden border-t-4 border-t-slate-800">
                ...
                >
                  <span className="flex items-center gap-3">
                    <i className="fa-solid fa-users text-xs opacity-40"></i>All
                    Members
                  </span>
                </button>
        ...
        {filteredTasks.length === 0 ? (
          <div className="bg-white p-20 rounded-[2.5rem] border border-slate-200 text-center shadow-sm">
            ...
            <p className="text-slate-400 mt-2">
              Adjust filters to browse archive.
            </p>
          </div>
        ) : (
          filteredTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="group bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden relative hover:shadow-xl hover:border-blue-200 transition-all"
              >
                {!isReadOnly && (
                  <button
                    onClick={() => onDeleteTask(task.id)}
                    className="absolute top-6 right-6 p-2 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all opacity-0 group-hover:opacity-100 z-10"
                  >
                ...
                <div className="p-8">
                    ...
                    <div className="flex gap-5">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100`}
                          >
                            {task.category || "General"}
                          </span>
                          <span
                            className={`text-[9px] px-2.5 py-1 rounded-full font-black uppercase tracking-tighter border ${statusStyles}`}
                          >
                            {latest?.status || "Assigned"}
                          </span>
                          <span
                            className={`text-[10px] font-bold uppercase tracking-tight ${timeInfo.color}`}
                          >
                            {timeInfo.label}
                          </span>
                        </div>
                        ...
                        <p className="text-sm text-slate-500 font-semibold mt-1 flex items-center gap-2">
                          <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center text-[8px] font-black">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div className="bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
                      <p className="text-[10px] text-slate-400 mb-2 uppercase font-black tracking-widest">
                        Latest Progress Note
                      </p>
                      <p className="text-sm text-slate-700 italic font-medium leading-relaxed">
                        {latest?.workCompleted
                          ? `"${latest.workCompleted}"`
                          : "Waiting for initial report..."}
                      </p>
                    </div>
                    <div className="bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
                      <p className="text-[10px] text-slate-400 mb-2 uppercase font-black tracking-widest">
                      ...
                      <div className="space-y-2 mt-2">
                        <div className="flex justify-between text-xs text-slate-600 font-bold">
                          <span className="flex items-center gap-2">
                            <i className="fa-solid fa-calendar-plus opacity-30"></i>{" "}
                            Assigned:
                          </span>
                          <span>
                  ...
                  <div className="mt-8 border-t border-slate-100 pt-6">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="p-1.5 bg-slate-100 rounded-lg">
                        <i className="fa-solid fa-history text-slate-400 text-xs"></i>
                      </div>
                    ...
                    <div className="space-y-3 mb-6">
                      {task.leadComments?.length ? (
                        task.leadComments.map((c, i) => (
                          <div
                            key={i}
                            className="text-sm bg-blue-50/20 text-blue-900 p-4 rounded-2xl border border-blue-100/30 flex items-start gap-3"
                          >
                            <span className="flex-1 font-medium italic leading-relaxed">
                              {c}
                            </span>
                          </div>
                        ))
                    ...
                    {!isReadOnly && (
                      <div className="flex gap-3">
                        <input
                          type="text"
                          placeholder="Add retrospective comment..."
                          className="flex-1 bg-white border border-slate-200 rounded-2xl px-5 py-3 text-sm font-medium focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                        ...
                        <button
                          ...
                          className="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-black transition-all shadow-md active:scale-95"
                        >
                          Append Note
                        </button>
                      </div>
